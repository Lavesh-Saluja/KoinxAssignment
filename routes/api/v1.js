...
router.get("/transactions", async (req, res) => {
   ...
   try{ 
    ...
    const url=`https://api.etherscan.io/api?module=account&action=txlist&address=${address}&startblock=0&endblock=99999999&apikey=${process.env.ETH_SCAN_API_KEY}`
       ...
       const user = await Transaction.findOne({ user: address });

       if (!user) {
           await Transaction.insertMany({user:address,transactions:result});
       }
       else {
           await Transaction.updateOne({ user: address }, { transactions: result });
       }
...
let totalWei = BigInt(0);
       ...
       let cnt = 0;
        for (const txn of weiValues) {
            const txnTo = txn.to.toLowerCase(); // Convert to lowercase for comparison
            const addressLower = address.toLowerCase(); // Convert to lowercase for comparison

           if (txnTo===addressLower)
           {
